...
def getInfo(codeA, year, i):
    ...
    phanlop = re.findall("([a-zA-Z]+)$", tenlop)
    ...
    code = post("http://localhost:8000/api/debug_profile", {
        "sbd" : sbd,
        "ten" : ten,
        "ngaysinh" : ngaysinh,
        "noisinh" : noisinh,
        "manganh" : codeA,
        "tennghanh" : codeClass[codeA],
        "lop" : phanlop,
        "khoa" : year
    })
...
for k in range(START_K, END_K+1):
    ...
    for c in codeClass:
        ...
        countThread += 1
        t = threading.Thread(target = getClass, args = (str(c), str(k), name))
        t.start()
